<template>
  <!-- <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/maska/dist/maska.css"> -->
      ...
      <table id="page_1" width="100%" border="0" style="border-collapse: collapse;display:table" bordercolor="#FFFFFF">
        <tr>
          ...
          <td width="40%" class="infoNormalFont" align="left">
            <select id="companySelect" v-model="selectedCompany" @change="handleCompanyChange" disabled>
        ...
        <tr>
          ...
          <td class="infoNormalFont" align="left" colspan="3">
            <input type="file" id="file_attach1" style="display: none;" @change="updateFileName" />
            <input type="button" value="Choose File" onclick="document.getElementById('file_attach1').click();" />
            <!-- <input type="file" id="file_attach1" name="file_attach1" accept="image/x-png,image/jpeg" >-->
            <div id="file_attach1_name"></div> 

            <!-- <input type="file" id="file_attach1" name="file_attach1" 
              accept="image/x-png,image/jpeg" ref="fileInput1" >  -->
              <!-- <span id="filename_display" style="color: red; font-size: 12; display: none;"></span> -->
              <span style="color:red; font-size:12"> ( ถ้ามี ต้องเป็นไฟล์รูป .jpg หรือ
              .png
              เท่านั้น)</span>
          </td>
            ...
            <table border="1" bordercolor="#767777" width="100%" style="font-size:14px">
              ...
              <tbody>
                <tr v-for="(rowData, index) in tableData" :key="index">
                  ...
                  <td align='left'>
                    <input v-maska:[maskOptionsMoney2digit] class='inputNumber' type='text'
                  ...
                  <table border="1" bordercolor="#767777" width="100%" style="font-size:14px">
                    ...
                    <tbody>
                      <tr v-for="(item, index) in Medal" :key="index">
                        ...
                        <td align="center">
                          <input v-maska:[maskOptionsMoney2digit] v-model="item['Amount']" class="inputNumber" type="text" @input="sumValue_Medal(index)" @change="forMatNumber_inputValue('Amount', item,0)"
                             id="Amount_{{ index }}" :name="'Amount_' + index"
                      ...
                      <tr>
                        ...
                        <td align="center">
                          <input v-model="formattedReserveMoney"
                                  v-maska:[maskOptionsMoney2digit]   class="inputNumber" type="text" 
...
<!-- <script type="importmap">
        {
          "imports": {
            "vue": "https://mydomain.com/vendors/vuejs-3.2.37/vue-debug.js",
            "maska": "https://cdn.jsdelivr.net/npm/maska@2/dist/maska.umd.js"
          }
        }
</script> -->
<script lang="ts">
...
// import { vMaska } from "maska"
// import { createMask } from 'maska';
// import 'maska/dist/maska.css';
...
export default defineComponent({
  ...
  setup() {
    ...
    // const input = document.getElementById('myInput');
      // Inputmask({ alias: 'decimal' }).mask(input);
    ...
    const OldfileInput1 = ref('');
    const updateFileNameCount = ref<number>(0);
    ...
    async function handleDepartment_1Change(index, TypeCode) {
      // Function logic here
      ...
      if (TypeCode) {
        // console.log(1)
        tableData.value[index - 1]['ประเภท'] = TypeCode;
      } else {
        // console.log(2)
    ...
    const onPageLoad = async () => {
      ...
      // console.log(id)
      await axios.get(`http://localhost:3100/api/csh_withdrawal_request_sea_edit/${id}`).then(async response => {
        ...
        if(data01.length > 0) {
          ...
          RequestDate.value = moment(data01[0]['RequestDate']).format('DD-MM-yyyy');
          UserNameReq.value = data01[0]['UserNameReq']
          Sum_Total.value = data01[0]['TotalRequest']
          // console.log(parseFloat(data01[0]['ReserveMoney']).toFixed(2))
          formattedReserveMoney.value = parseFloat(data01[0]['ReserveMoney']).toFixed(2).toString()
          // console.log(data01[0]['AttachFile1'])
          if (data01[0]['AttachFile1']) {
            OldfileInput1.value = data01[0]['AttachFile1']
            const attachFileName = data01[0]['AttachFile1'];
            const newFileName = attachFileName.replace(/_\d{8}_\d{6}\./, ".");
            // Set the file name in the div or span element
            const fileAttach1NameElement = document.getElementById('file_attach1_name');
            if (fileAttach1NameElement) {
              fileAttach1NameElement.textContent = newFileName;
            }
            // Assuming you have an input element with id 'file_attach1'
            const fileInput = document.getElementById('file_attach1') as HTMLInputElement;
            
            // Construct the full file path
            const filePath = `D:/working_project/Tomcat_8.5/webapps/petty_cash/attach/images/${data01[0]['AttachFile1']}`;
            ...
            // Set the FileList as the files of the file input
            
            fileInput.files = fileList.files;
            // fileInput.files.item.name = 'XXXX';
            // console.log(fileInput)
            // fileInput.src = data01[0]['AttachFile1']
          }
          const data10 = response.data.data['10'];

          if (data10.length > 0) {
            const formattedTableData = data10.map((item) => ({
              'เลขใบเบิก': item['RequestDocNo'],
              'รายละเอียด': item['Description'],
              'แผนก': item['Department'],
              'ประเภท': item['TypeCode'],
              'ยอดก่อน VAT': item['BeforVat'],
              'VAT': item['VAT'],
              'ภาษี ณ ที่จ่าย': item['WHT'],
              'รวม': item['Total'],
              'เลขสัญญาเช่า': item['RefDoc'],
              'ทะเบียนรถ': item['VehicleNumber'],
              'APRDC': []
            }));

            // Assign the formatted data to tableData
            tableData.value = formattedTableData;
            for (let i = 0; i < formattedTableData.length; i++) {
              // console.log(formattedTableData[i]['ประเภท'])
              await handleDepartment_1Change(i+1,formattedTableData[i]?.['ประเภท'])
            }
          } else {
    ...
    const onSubmitSaveChanges = async () => {
      ...
      const requestData = {
        REQUEST_CASHH: {
          ...
          ReserveMoney: ReserveMoney.value,
          AttachFile1: OldfileInput1.value,
          IsDelete: '0'
        },
        REQUEST_CASHD: tableData.value.map((item, index) => ({
          ...
          Branch: '',
          BeforVat: parseNumber(item['ยอดก่อน VAT']),
          VAT: parseNumber(item['VAT']),
          Total: parseNumber(item['รวม']),
          WHT: parseNumber(item['ภาษี ณ ที่จ่าย']),
      ...
      // console.log(requestData)
       if (updateFileNameCount.value > 0) {
        const fileInput = document.getElementById('file_attach1') as HTMLInputElement;
        ...
        if (fileInput.files && fileInput.files.length > 0) {
          ...
          requestData.REQUEST_CASHH.AttachFile1 = uniqueFileName;
          ...
          // formData.append('file_attach1', fileInput.files[0]);        
            ...
            await axios.put(`http://localhost:3100/api/upload_image?uniqueFileName=${uniqueFileName}&OldfileInput1=${OldfileInput1.value}`, formData, {
              headers: {
      ...
      try {        
        ...
        if(status == true) {
          await axios.put(`http://localhost:3100/api/csh_withdrawal_request_sea_edit/${id}`, requestData)
          .then(response => {
            // console.log(response)
            const responseData = response.data; 
          })
          .catch(error => {
            axios.delete(`http://localhost:3100/api/del_image/${requestData.REQUEST_CASHH.AttachFile1}`);
    ...
    function updateFileName() {
      updateFileNameCount.value = updateFileNameCount.value + 1
      console.log(updateFileNameCount.value)
      const fileInput = document.getElementById('file_attach1') as HTMLInputElement;
      const fileNameDisplay = document.getElementById('file_attach1_name') as HTMLDivElement;
    ...
    const maskOptionsMoney2digit = ref({
      mask: '0.9999',
