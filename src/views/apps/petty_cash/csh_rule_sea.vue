  ...
  <div :class="widgetClasses" class="card">
    ...
    <div class="card-header border-0 pt-6">
      ...
      <div class="card-toolbar">
        <div v-if="selectedIds.length === 0" class="d-flex justify-content-end" data-kt-subscription-table-toolbar="base">
          <router-link :to="'csh_rule_add'" class="btn btn-primary">
    ...
    <div class="card-body py-3">
      ...
      <!-- <div class="table-responsive"> -->
        <!--begin::Table-->
        <Datatable @on-sort="sort" @on-items-select="onItemSelect" :loading="loading" :data="UserRulesList"
              :header="headerConfig" :enable-items-per-page-dropdown="true" :checkbox-enabled="false" checkbox-label="UserRule_Id">
              <!-- <template v-slot:email="{ row: account }"><a href="#" class="text-dark fw-bold text-hover-primary fs-6">
                  {{ account.email }}</a>
              </template> -->
              <template v-slot:UserRule_Id="{ row: account }">
                <div class="badge badge-light">{{ account.UserRule_Id }}</div>
              </template> 
              ...
              <router-link :to="'csh_rule_edit/' + account.UserRule_Id"
                class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                <KTIcon icon-name="pencil" icon-class="fs-3" />
              </router-link>
              <a @click="deleteButton(account.UserRule_Id)" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm">
...
import { UserRulesStore, type UserRules } from "@/stores/UserRules";
...
export default defineComponent({
  ...
  setup() {
    const headerConfig = ref([
      { columnName: "ID", columnLabel: "UserRule_Id" },      
      { columnName: "สิทธิ์", columnLabel: "UserRule_Name" },      
     { columnName: "Actions", columnLabel: "actions" },
    ]);
    const selectedIds = ref<Array<number>>([]);
    const store = UserRulesStore();
    const UserRulesList = ref<Array<UserRules>>([]);
    const initUserRulesList = ref<Array<UserRules>>([]);
    ...
    const onDataTable = async () => {
      // UserRulesList = null
      UserRulesList.value = await store.getList();
      // console.log("UserRulesList.value")
      // console.log(UserRulesList.value)
      loading.value = false;
      initUserRulesList.value = await store.getList();
    }
    const sort = (sort: Sort) => {
      const reverse: boolean = sort.order === "asc";
      if (sort.label) {
        arraySort(UserRulesList.value, sort.label, { reverse });
      }
    };
      ...
      Swal.fire({
        ...
        .then(async function (result: any,actions: any) {
          if (result.value) {
            ...
            await axios.put(`http://localhost:3100/api/updateUserRules/${id}`, requestData)
                .then(response => {
                  // actions.resetForm()
                    Swal.fire({
    ...
    const searchItems = () => {
      UserRulesList.value.splice(0, UserRulesList.value.length, ...initUserRulesList.value);
      if (search.value !== "") {
        let results: Array<any> = [];
        for (let j = 0; j < UserRulesList.value.length; j++) {
          if (searchingFunc(UserRulesList.value[j], search.value)) {
            results.push(UserRulesList.value[j]);
          }
        }
        UserRulesList.value.splice(0, UserRulesList.value.length, ...results);
      }
    };
    ...
    const deleteSubscription = (UserRule_Id: number) => {
      for (let i = 0; i < UserRulesList.value.length; i++) {
        if (UserRulesList.value[i].UserRule_Id === UserRule_Id) {
          UserRulesList.value.splice(i, 1);
        }
      }
    };
    ...
    // const products = ref<csh_user_list[]>([]);
    // const fetchUsers = async () => {
    // try {
    //   const response = await axios.get(`${import.meta.env.VITE_APP_API_TRG}/csh_rule_sea`);
    //   products.value = response.data; // Assuming the response.data is an array of User objects
    // } catch (error) {
    //   console.log(error);
    // }
    // };
    ...
    // axios.get('http://localhost:3000/api/UserAdmin').then(data => products.value = data);

    // fetch('http://localhost:3000/api/UserAdmin')
